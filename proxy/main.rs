...
fn main() {
    ...
    let matches = App::new("rvncproxy")
        .about("VNC proxy")
        .arg(
            Arg::with_name("CONNECT-HOST")
                .help("server hostname or IP")
                .required(true)
                .index(1),
        )
        .arg(
            Arg::with_name("CONNECT-PORT")
    ...
    let connect_host = matches.value_of("CONNECT-HOST").unwrap();
    let connect_port = value_t!(matches.value_of("CONNECT-PORT"), u16).unwrap_or(5900);
    let listen_host = matches.value_of("LISTEN-HOST").unwrap_or("localhost");
    let listen_port = value_t!(matches.value_of("LISTEN-PORT"), u16).unwrap_or(connect_port + 1);

    info!("listening at {}:{}", listen_host, listen_port);
    let listener = match std::net::TcpListener::bind((listen_host, listen_port)) {
    ...
    for incoming_stream in listener.incoming() {
        ...
        let server_stream = match std::net::TcpStream::connect((connect_host, connect_port)) {
            Ok(stream) => stream,
            Err(error) => {
                error!(
                    "cannot connect to {}:{}: {}",
